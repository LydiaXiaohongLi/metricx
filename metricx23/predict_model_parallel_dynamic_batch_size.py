...
def create_dataloaders(path, tokenizer, is_qe, max_input_length, input_lengths, batch_sizes):
    ...
    if is_qe:
        data = [{**{"input": f'candidate: {example["hypothesis"]} source: {example["source"]}', "input_file_order": i}, **example} for i, example in enumerate(data)]
    else:
        data = [{**{"input": f'candidate: {example["hypothesis"]} reference: {example["reference"]}', "input_file_order": i}, **example} for i, example in enumerate(data)]
